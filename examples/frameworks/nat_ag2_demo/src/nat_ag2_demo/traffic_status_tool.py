...
"""Los Angeles traffic status tool with time-of-day awareness."""
...
class TrafficStatusToolConfig(FunctionBaseConfig, name="traffic_status_ag2"):
...
@register_function(config_type=TrafficStatusToolConfig, framework_wrappers=[LLMFrameworkEnum.AG2])
async def traffic_status(_config: TrafficStatusToolConfig, _builder: Builder) -> AsyncIterator[FunctionInfo]:
    ...
    Args:
        _config: The configuration for the traffic status tool.
        _builder: The NAT builder instance.

    Yields:
        FunctionInfo wrapping the traffic status tool.
    """
    ...
    async def _traffic_status(hwy: str, hour: int) -> str:
        ...
        return TRAFFIC_DATA[hwy_key][period]
