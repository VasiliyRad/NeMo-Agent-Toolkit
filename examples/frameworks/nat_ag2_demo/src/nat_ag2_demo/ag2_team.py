...
class AG2TeamConfig(FunctionBaseConfig, name="ag2_team"):
    """Configuration for the AG2 multi-agent team workflow."""
...
@register_function(config_type=AG2TeamConfig, framework_wrappers=[LLMFrameworkEnum.AG2])
async def ag2_team(config: AG2TeamConfig, builder: Builder) -> AsyncIterator[FunctionInfo]:
    """AG2 multi-agent team workflow using ConversableAgent and AutoPattern.
    ...
    try:
        ...
        async def _ag2_team_workflow(user_input: str) -> str:
            """Execute the AG2 AutoPattern group chat workflow.
            ...
            """
            try:
                # Query processing agent: knows about tools via LLM registration
                ...
                # Final response agent: formats the collected data, no tools
                ...
                # User agent: initiates the chat and executes tool calls
                user = ConversableAgent(name="user", human_input_mode="NEVER")

                # Register tools: query_agent's LLM knows about them, user executes them
                for tool in tools:
        ...
        logger.info("AG2 team workflow exited early")
    except Exception as e:
        logger.error("Failed to initialize AG2 team workflow: %s", e)
        raise
    finally:
        logger.debug("AG2 team workflow cleanup completed")
